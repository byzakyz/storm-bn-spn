...
ProbabilityTable::ProbabilityTable(const BNNode& n, const std::vector<BNNode>& ps, const std::vector<ProbabilityRow>& rows){
...
ProbabilityTable::ProbabilityTable(std::string declaration, std::map<std::string, BNNode> nodes) {
...
void ProbabilityTable::parseRows() {
    Utils util;
    BIF bif;
    std::string match_string = util.returnMatchedString(tableDeclaration, "[{]" + bif.SPACESTAR + bif.probabilityContent + bif.SPACESTAR + "[}]");
    ...
    while ((pos = s.find(delimiter)) != std::string::npos) {
        ...
        ProbabilityRow row(token, parentsNames, node.getVariableValueList());
